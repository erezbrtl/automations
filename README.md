...
| `formEndpoint` | כתובת Webhook שמקבלת את הטפסים ב-POST/JSON (Make, n8n, Formspree, Zapier). **עדיין ריק.** |
| `contactEmail` | כתובת מייל לגיבוי אחרון. מולא. |
| `whatsapp` | מספר בפורמט בינלאומי בלי `+`. מולא - מפעיל את כפתור הוואטסאפ הצף ואת הקישור בטופס. |

### לאן הולכת פנייה

הטופס מנסה שלושה מסלולים לפי הסדר:

1. **`formEndpoint`** - שליחה שקטה ב-POST. זה המסלול הרצוי.
2. **וואטסאפ** - אם אין endpoint, נפתחת שיחה עם הפרטים כבר מוכנים בהודעה. המשתמש רק לוחץ שלח.
3. **מייל** - אם אין גם וואטסאפ.

אם אף אחד מהשלושה לא מוגדר, **הטופס לא מציג אישור** אלא הודעת שגיאה גלויה, והפנייה לא נעלמת בשקט.

מומלץ להוסיף `formEndpoint` (Formspree בחינם, עשר דקות) כדי שהפניות יישמרו גם אם המשתמש נטש באמצע.

### מדידה

בכל שליחה מוצלחת נורים שלושה אירועים, בלי לטעון שום ספרייה חיצונית:
`dataLayer.push({event:"lead_submit"})` ל-GTM, `gtag("event","generate_lead")` אם GA4 טעון,
ואירוע DOM בשם `lead:submit` לכל מאזין אחר.
...
הקובץ `resources/processes.html` נשאר במקום; צריך רק לקשר אליו ולהחזיר טופס.
...
resources/processes.html      רשימת 12 התהליכים (מותאמת להדפסה) - קיימת אך לא מקושרת מהעמוד
...
- **טיפוגרפיה** - `Rubik` בכל האתר: כותרות, טקסט רץ וכפתורים. משקלים 400/500/600/700/800.
